   ...
   private boolean dayChecker(ArrayList<Booking> list, Date startDate, Date endDate) {
      if (list.size() == 0) {
         ...
         for (int i = 0; i < list.size(); i++) {
            if (startDate.compareTo(bookings.get(i).getStartDate()) <= 0
                  && endDate.compareTo(bookings.get(i).getEndDate()) >= 0)
               return false;
            else if (startDate.compareTo(bookings.get(i).getStartDate()) >= 0
                  && endDate.compareTo(bookings.get(i).getEndDate()) <= 0)
               return false;
            else if ((startDate.compareTo(bookings.get(i).getStartDate()) >= 0 && startDate
                  .compareTo(bookings.get(i).getEndDate()) <= 0)
                  && endDate.compareTo(bookings.get(i).getEndDate()) >= 0)
               return false;
            else if (startDate.compareTo(bookings.get(i).getStartDate()) <= 0
                  && (endDate.compareTo(bookings.get(i).getStartDate()) >= 0 && endDate
                  .compareTo(bookings.get(i).getEndDate()) <= 0))
               return false;
         }
         return true;
