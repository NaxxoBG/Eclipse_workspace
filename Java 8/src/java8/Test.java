...
public class Test
{
   ...
   public static void main(String[] args) {
      ...
      System.out.println("Sorting objects by one of their fields -> ");
      List<Person> people = new ArrayList<Person>(Arrays.asList(p1, p2));
      ...
      String reversedString = new StringBuilder(message).reverse().toString();
      System.out.println(reversedString);

      //Reverse an int[] array
      int[] nums = {25, 4, 60, 7};
      int[] revNums = IntStream.rangeClosed(1, nums.length).map(i -> nums[nums.length - i]).toArray();
      System.out.println(Arrays.toString(revNums));

      //Covert an iterator to a stream
      ...
      Stream.concat(stream, Stream.empty());
